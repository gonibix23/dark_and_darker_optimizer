...
class Character(object):
    ...
    def calculate_stats(self):
        self.max_health = (stats.max_health(self.vigor*0.75+self.strength*0.25)+self.additional_max_health)*self.max_health_bonus
        self.health_recovery = stats.health_recovery(self.vigor)
        
        self.move_speed = stats.move_speed(self.agility+self.move_speed_bonus)+300

        self.item_equip_speed = stats.item_equip_speed(self.dexterity)
        self.manual_dexterity = stats.manual_dexterity(self.dexterity)
        self.magic_resist = stats.magic_resistance(self.will)+self.magic_resist
        
        self.magical_power_bonus = stats.magical_power_bonus(self.will+self.magical_power)+self.magical_damage_bonus
        self.physical_power_bonus = stats.physical_power_bonus(self.strength+self.physical_power)+self.physical_damage_bonus

        self.buff_duration = stats.buff_duration(self.will)+self.additional_buff_duration
        self.debuff_duration = stats.debuff_duration(self.will)+self.additional_debuff_duration
        self.persuasiveness = stats.persuasiveness(self.resourcfulness)

        self.spell_casting_speed = stats.spell_casting_speed(self.knowledge)
        self.memory_capacity = stats.memory_capacity(self.knowledge)
        self.memory_recovery = stats.memory_recovery(self.knowledge)

        self.action_speed = stats.action_speed(self.agility*0.25+self.dexterity*0.75)+self.additional_action_speed

        self.magical_interaction_speed = stats.magical_interaction_speed(self.will)+self.additional_magic_interaction_speed
        self.regular_interaction_speed = stats.regular_interaction_speed(self.agility*0.4+self.resourcfulness*0.6)+self.additional_regular_interaction_speed
...
class Rogue(Character):
    def __init__(self):
        ...
        self.armor = -10 # Not working this override
